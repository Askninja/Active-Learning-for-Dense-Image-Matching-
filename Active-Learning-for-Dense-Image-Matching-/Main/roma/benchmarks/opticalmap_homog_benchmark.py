...
class OpticalmapHomogBenchmark:
    ...
    def benchmark(self, model, model_name=None, vis=False, thresh_score=0.05):
        ...
        for count, idx in enumerate(self.test_idx):
            ...
            all_epe.append(epe)
            # Homography-stability certainty for correlation with EPE.
            if pos_a.shape[0] >= 8:
                ...
                for _ in range(50):
                    sel = (
                        g.choice(pos_a.shape[0], size=subset, replace=False)
                        if pos_a.shape[0] > subset
                        else np.arange(pos_a.shape[0])
                    )
                    pA, pB = pos_a[sel], pos_b[sel]
                    ...
                    if H is not None and abs(H[2, 2]) > 1e-12:
                        Hs.append(H / (H[2, 2] + 1e-12))
                if len(Hs) < 2:
                    hs_cert_vals.append(0.0)
                else:
                    Hs = np.stack(Hs, axis=0)
                    corners = np.float32([[0, 0], [w1 - 1, 0], [w1 - 1, h1 - 1], [0, h1 - 1]]).reshape(-1, 1, 2)
                    warped = np.stack([cv2.perspectiveTransform(corners, H).reshape(4, 2) for H in Hs], axis=0)
                    s = float(warped.std(axis=0).mean())
                    hs_cert_vals.append(1.0 / (1.0 + max(s, 0.0)))
            else:
                hs_cert_vals.append(0.0)
            uncert = 'default'
        ...
        hs_cert_vals = np.asarray(hs_cert_vals, dtype=float)
        all_epe_arr = np.asarray(all_epe, dtype=float)
        hs_epe_pearson = np.nan
        if hs_cert_vals.size > 1 and all_epe_arr.size == hs_cert_vals.size:
            try:
                hs_epe_pearson = float(pearsonr(hs_cert_vals, all_epe_arr)[0])
            except Exception:
                hs_epe_pearson = np.nan
        return {
            "auc_3": auc[2],
            ...
            "auc_10": auc[9],
            "epe": np.mean(all_epe),
            "hs_cert_mean": float(hs_cert_vals.mean()) if hs_cert_vals.size > 0 else np.nan,
            "hs_cert_epe_pearson": hs_epe_pearson,
        }
