...
import roma
from roma.utils import get_tuple_transform_ops, get_tuple_transform_ops_my, get_depth_tuple_transform_ops
...
from roma.losses.robust_loss import RobustLosses, RobustLossesMy
...
class ConvRefinerMy(ConvRefiner):
    def forward(self, x, y, flow, scale_factor = 1, logits = None):
        b,c,hs,ws = x.shape
        with torch.autocast("cuda", enabled=self.amp, dtype = self.amp_dtype):
            with torch.no_grad():
            ...
            d = self.block1(d)
            d = self.hidden_blocks(d)
        d = self.out_conv(d.float()) # 0,1: displacement; 2: certainty; 3,4: la; 5,6: alpha; 7,8: beta
        displacement, certainty = d[:, 0:2], d[:, 2:4]
        return displacement, certainty
...
class Decoder(nn.Module):
    ...
    def forward(self, f1, f2, gt_warp = None, gt_prob = None, upsample = False, flow = None, certainty = None, scale_factor = 1, **kwargs):
...
class DecoderMy(Decoder):
    def forward(self, f1, f2, gt_warp = None, gt_prob = None, upsample = False, flow = None, certainty = None, scale_factor = 1, **kwargs):
        coarse_scales = self.embedding_decoder.scales()
        #print('coarse_scales is', coarse_scales)
        all_scales = self.scales if not upsample else ["8", "4", "2", "1"] 
        sizes = {scale: f1[scale].shape[-2:] for scale in f1}
        h, w = sizes[1]
        b = f1[1].shape[0]
        ...
        for new_scale in all_scales:
            ...
            if new_scale in self.conv_refiner:
                ...
                #print('delta_certainty shape', delta_certainty.shape)
                certainty = (
                    certainty + delta_certainty
                )  # predict both certainty and displacement
...
class RegressionMatcher(nn.Module):
    ...
    def sample(
        ...
        if "threshold" in self.sample_mode:
            ...
            upper_thresh = thresh_score
            certainty = certainty.clone()
            certainty0 = certainty.clone().reshape(-1)
    ...
    def forward(self, batch, batched = True, upsample = False, scale_factor = 1):
        if self.symmetric:
            return self.forward_symmetric(batch)
        ...
        with torch.no_grad():
            ...
            if self.upsample_preds:
                ...
                if self.recrop_upsample:
                    ...
                    if isinstance(im_A_path, (str, os.PathLike)):
                        im_A, im_B = Image.open(im_A_path).convert("RGB"), Image.open(im_B_path).convert("RGB")
                        test_transform = get_tuple_transform_ops_my(resize=(hs, ws), normalize=True)
                    else:
                        # Assume its not a path
                        im_A, im_B = im_A_path, im_B_path
                        test_transform = get_tuple_transform_ops_my(resize=(hs, ws), normalize=False)
                #print('im_A shape before transform is', im_A.shape)
                
                im_A, im_B = test_transform((im_A, im_B))
                if not batched:
                   im_A, im_B = im_A[None], im_B[None] 
                im_A, im_B = im_A.to(device), im_B.to(device)

                scale_factor = math.sqrt(self.upsample_res[0] * self.upsample_res[1] / (self.w_resized * self.h_resized))
                batch = {"im_A": im_A, "im_B": im_B, "corresps": finest_corresps}
                #print('im_A shape after transform is', im_A.shape)
                if symmetric:
    ...
    def match_my(
        ...
        *args,
        ...
        with torch.no_grad():
            ...
            #print(corresps.keys())
            im_A_to_im_B = corresps[finest_scale]["flow"] 
            #print('self.attenuate_cert is', self.attenuate_cert)
            #certainty = corresps[finest_scale]["certainty"] - (low_res_certainty if self.attenuate_cert else 0)
            certainty = corresps[finest_scale]["certainty"]
            ...
            if (im_A_to_im_B.abs() > 1).any() and True:
                wrong = (im_A_to_im_B.abs() > 1).sum(dim=-1) > 0
                el_certainty[wrong[:,None]] = 0
                mask_wrong = wrong.unsqueeze(1).repeat(1, 2, 1, 1)
                #print("p shape is", p.shape)
                #print("mask_wrong shape is", mask_wrong.shape)
                p[mask_wrong] = 0
            ...
            if symmetric:
                ...
                s_warp = torch.cat((B_to_A, im_B_coords), dim=-1)
                warp = torch.cat((q_warp, s_warp),dim=2)
                el_certainty = torch.cat(el_certainty.chunk(2), dim=3)
                p = torch.cat(p.chunk(2), dim=3)
            else:
                warp = torch.cat((im_A_coords, im_A_to_im_B), dim=-1)
            if batched:
                return (
                ...
                #print('warp shape is', warp.shape)
                #print('certainty shape is', certainty.shape)
    ...
    def scale_intrinsic(self, K, wi, hi):
        sx, sy = self.w_resized / wi, self.h_resized / hi
        sK = torch.tensor([[sx, 0, 0], [0, sy, 0], [0, 0, 1]])
        return sK @ K
    
    def fgsm_attack_old(self, image, epsilon, data_grad):
    ...
    def fgsm_attack(self, batch, loss_func, epsilon):
        batch['im_A'].requires_grad = True   
        batch['im_B'].requires_grad = True

        corresps = self.forward(batch)
        loss = loss_func(corresps, batch)
       
        self.zero_grad()
       
        loss.backward()

        adv_imA = batch['im_A'] + epsilon * batch['im_A'].grad.sign()
        adv_imB = batch['im_B'] + epsilon * batch['im_B'].grad.sign()
        return adv_imA, adv_imB

    def pgd_attack(self, batch, loss_func, epsilon = 0.3, alpha=2/255, iters=20) :
       
            
        ori_imA = batch['im_A'].clone().detach()
        ori_imB = batch['im_B'].clone().detach()

        alpha = (epsilon + 0.1) / iters   
        for i in range(iters) :  
            ...
            adv_imA = batch['im_A'] + alpha*batch['im_A'].grad.sign()
            eta = torch.clamp(adv_imA - ori_imA, min = -epsilon, max = epsilon)
            #batch['im_A'] = torch.clamp(ori_imA + eta, min=-1, max=1).detach_()
            batch['im_A'] = (ori_imA + eta).detach_()

            adv_imB = batch['im_B'] + alpha*batch['im_B'].grad.sign()
            eta = torch.clamp(adv_imB - ori_imB, min = -epsilon, max = epsilon)
            #batch['im_B'] = torch.clamp(ori_imB + eta, min=-1, max=1).detach_()
            batch['im_B'] = (ori_imB + eta).detach_()
    ...
    def match_my_adv(
        ...
        if not batched:
            ...
            depth_transform_ops = get_depth_tuple_transform_ops(resize=(hs, ws))
            
            im_A_depth, im_B_depth = depth_transform_ops((im_A_depth[None, None], im_B_depth[None, None]))
            K1 = self.scale_intrinsic(K1, w, h)
            K2 = self.scale_intrinsic(K2, w2, h2)
            im_A = im_A[None].to(device)
            im_B = im_B[None].to(device)
            #im_A0 = torch.tensor(im_A[None], device="cuda", requires_grad=True)
            #im_B0 = torch.tensor(im_B[None], device="cuda", requires_grad=True)
            batch = {"im_A": im_A, "im_B": im_B, 
                     "im_A_depth": im_A_depth[0].to(device), "im_B_depth": im_B_depth[0].to(device), "T_1to2": T_1to2[None].to(device),
                     "K1":K1[None].to(device), "K2": K2[None].to(device)}
        else:
            b, c, h, w = im_A.shape
            b, c, h2, w2 = im_B.shape
            assert w == w2 and h == h2, "For batched images we assume same size"
        ...
        if corruption == 'fgsm':
            im_A_perturbed, im_B_perturbed = self.fgsm_attack(batch, depth_loss, epsilon)
        elif corruption == 'pgd':
            im_A_perturbed, im_B_perturbed = self.pgd_attack(batch, depth_loss, epsilon)
        else:
            im_A_perturbed, im_B_perturbed = im_A, im_B
            
        batch = {"im_A": im_A_perturbed, "im_B": im_B_perturbed}
        torch.cuda.empty_cache()
        gc.collect()
        ...
        with torch.no_grad():
            ...
            if self.upsample_preds:
                finest_corresps = corresps[finest_scale]
                torch.cuda.empty_cache()
                test_transform = get_tuple_transform_ops_my(
    ...
    def match_scannet_adv(
        ...
        if el:
            depth_loss_mega = RobustLossesMy(
                ce_weight = 0, 
                local_dist={1:4, 2:4, 4:8, 8:8},
                local_largest_scale=8,
                depth_interpolation_mode = 'bilinear',
                ...
                ua_weight = 0.01,
                kl_weight = 1)
            depth_loss_scannet = RobustLossesMy(
                ce_weight = 0, 
                local_dist={1:4, 2:4, 4:8, 8:8},
                local_largest_scale=8,
                depth_interpolation_mode = 'bilinear',
        ...
        if corruption == 'fgsm':
            im_A_perturbed, im_B_perturbed = self.fgsm_attack(batch, depth_loss_scannet, epsilon)
        elif corruption == 'pgd':
            im_A_perturbed, im_B_perturbed = self.pgd_attack(batch, depth_loss_scannet, epsilon)
