...
import roma
from roma.utils import get_tuple_transform_ops, get_tuple_transform_ops_my
...
class ConvRefinerMy(ConvRefiner):
    def forward(self, x, y, flow, scale_factor = 1, logits = None):
        ...
        displacement, certainty = d[:, 0:2], d[:, 2:3]
        la, alpha, beta = F.softplus(d[:, 3:5]), F.softplus(d[:, 5:7]) + 1, F.softplus(d[:, 7:9])
        return displacement, certainty, la, alpha, beta
...
class DecoderMy(Decoder):
    def forward(self, f1, f2, gt_warp = None, gt_prob = None, upsample = False, flow = None, certainty = None, scale_factor = 1, **kwargs):
        ...
        for new_scale in all_scales:
            ...
            if new_scale in self.conv_refiner:
                corresps[ins].update({"flow_pre_delta": flow}) if self.training else None
                delta_flow, delta_certainty, la, alpha, beta = self.conv_refiner[new_scale](
                    f1_s, f2_s, flow, scale_factor = scale_factor, logits = certainty,
                )                    
                corresps[ins].update({"delta_flow": delta_flow,}) if self.training else None
                corresps[ins].update({"la": la}) 
                corresps[ins].update({"alpha": alpha}) 
                corresps[ins].update({"beta": beta}) 
                
                print(f'scale {new_scale} la max is {la.max()}')
                print(f'scale {new_scale} la min is {la.min()}')

                print(f'scale {new_scale} alpha max is {alpha.max()}')
                print(f'scale {new_scale} alpha min is {alpha.min()}')

                print(f'scale {new_scale} beta max is {beta.max()}')
                print(f'scale {new_scale} beta min is {beta.min()}')

                al = beta / (alpha - 1 + 1e-5)
                ep = beta / (la * (alpha - 1 + 1e-5))

                corresps[ins].update({"al": al})
                corresps[ins].update({"ep": ep})
...
class RegressionMatcher(nn.Module):
    ...
    def get_el_uncertainty_merge(self, corresps):
        ...
        for scale in scales:
            ...
            flow, scale_la, scale_alpha, scale_beta = (
                scale_corresps["flow"],
            ...
            ests.append([scale_flow, scale_la, scale_alpha, scale_beta])
        ...
        al = torch.sum(al, dim = 1, keepdim = True)
        ep = torch.sum(ep, dim = 1, keepdim = True)
        return ep, al
    
    def get_el_uncertainty_scale(self, corresps, scale = 1): 
        hs, ws = corresps[1]["certainty"].shape[-2:]
        al = torch.sum(corresps[scale]["al"], dim = 1, keepdim = True)
        ep = torch.sum(corresps[scale]["ep"], dim = 1, keepdim = True)

           
        if scale != 1:
            al = F.interpolate(
            al, size=(hs, ws), align_corners=False, mode="bilinear"
            )
            ep = F.interpolate(
            ep, size=(hs, ws), align_corners=False, mode="bilinear"
            )
    ...
    def match_my(
        ...
        *args,
        ...
        with torch.no_grad():
            ...
            #ep, al = self.get_el_uncertainty_merge(corresps)

            ep, al = self.get_el_uncertainty_scale(corresps)

            print('ep max is', ep.max())
            print('ep min is', ep.min())
            ...
            if (im_A_to_im_B.abs() > 1).any() and True:
                wrong = (im_A_to_im_B.abs() > 1).sum(dim=-1) > 0
                certainty[wrong[:,None]] = 0
                al[wrong[:,None]] = al.max()
                ep[wrong[:,None]] = ep.max()
            ...
            if symmetric:
                ...
                warp = torch.cat((q_warp, s_warp),dim=2)
                certainty = torch.cat(certainty.chunk(2), dim=3)
                al = torch.cat(al.chunk(2), dim=3)
                ep = torch.cat(ep.chunk(2), dim=3)
            else:
                warp = torch.cat((im_A_coords, im_A_to_im_B), dim=-1)
            if batched:
                return (
