...
import os, os.path as osp, numpy as np, torch, torch.nn.functional as F, cv2, time
...
class ActiveLearningStrategy:
    def __init__(self, args, cycle: int, data_root, split: str, idx_root=None, rng_seed: int = 784) -> None:
        ...
        self.dataset_name = getattr(args, "dataset_name", "opticalmap")
        self.strategy = getattr(args, "strategy")
        self.train_resolution = getattr(args, "train_resolution", "low")
        self.temperature = float(getattr(args, "entropy_temperature", 0.5))
        self.selector_batch_size = int(getattr(args, "selector_batch_size", 3))
        ...
        self.preseed_idx = self._load_idx("preseed", required=False)
    ...
    def _budget_for_cycle(self) -> int:
        if self.cycle < 0:
            return 0
        schedules = {
            "MSRS":        {0: 75,  1: 75,  2: 75,  3: 150},
            "DPDN":        {0: 50,  1: 50,  2: 50,  3: 100},
            "Optical-SAR": {0: 200, 1: 200, 2: 200, 3: 400},
        }
        schedule = schedules.get(self.dataset_name, {0: 10, 1: 10, 2: 10, 3: 20})
        return int(schedule.get(self.cycle, schedule.get(3, 25)))
    ...
    def _create_remain_dataset(self, idx_subset: np.ndarray):
        resolutions = {
            "low": (448, 448),
    ...
    def _pair_embedding(self, model, batch) -> np.ndarray:
        ...
        for scale in sorted_scales:
            features = feature_pyramid[scale]           # (2, C, H, W)
            feat_a, feat_b = features.chunk(2, dim=0)  # each (1, C, H, W)
            gap_a = F.normalize(feat_a.flatten(2).mean(dim=2), dim=-1)  # (1, C)
            gap_b = F.normalize(feat_b.flatten(2).mean(dim=2), dim=-1)  # (1, C)
            gaps_a.append(gap_a)
            gaps_b.append(gap_b)
        emb_a = torch.cat(gaps_a, dim=1)  # (1, num_scales * C)
        emb_b = torch.cat(gaps_b, dim=1)  # (1, num_scales * C)
        return torch.cat((emb_a, emb_b), dim=1)[0].detach().float().cpu().numpy()
    ...
    def _compute_fine_feature_embeddings(
        ...
        """
        dataset = self._create_remain_dataset(avail)
        dataloader = DataLoader(dataset, batch_size=max(1, self.selector_batch_size), shuffle=False, num_workers=0, pin_memory=True)
        ...
        with torch.no_grad():
            for _, batch in zip(avail.astype(int), dataloader):
                ...
                emb = self._pair_embedding(model, batch)
                embeddings.append(emb)
        if not embeddings:
            raise ValueError(f"_compute_fine_feature_embeddings: no embeddings produced for avail={avail}")
    ...
    #         raise ValueError(f"_compute_geometry_descriptors: idx_subset is empty")
    #     dataset = self._create_remain_dataset(idx_subset)
    ...
    #     if not descriptors:
    #         raise ValueError(f"_compute_geometry_descriptors: dataloader yielded no batches for idx_subset={idx_subset}")
    ...
    def _get_matches_and_confidences(
        ...
        """
        from torch.utils.data import DataLoader
        dataset = self._create_remain_dataset(np.array([pair_id], dtype=int))
