...
Faithful implementation of He et al. TMLR 2024:
  ...
  - u(x): entropy score already in [0,1] (NOT remapped to [1,2])
...
def run(strategy, k: int, model) -> np.ndarray:
    ...
    Implements UWE from He et al. TMLR 2024:
        h(x) = u(x) · f(x)
    where u(x) ∈ [0,1] is the entropy uncertainty and f(x) is the raw
    (unnormalized) backbone feature.  k-center greedy is then run on h(x)
    directly so that uncertain pairs sit farther from the origin and are
    preferentially selected.

    When labeled pairs exist the greedy search is seeded from their
    weighted embeddings (h_lab) so that selected pairs are diverse relative
    to what is already labeled — at the same scale.
    ...
    # Entropy from gm_cls is already normalized by log(4097) → [0,1]
    # ------------------------------------------------------------------
    score_ids, entropy_scores = strategy._score_avail(
        model, avail, score_name="entropy"
    )
    score_map = {int(sid): float(s) for sid, s in zip(score_ids.tolist(), entropy_scores.tolist())}
    u = np.array(
        [score_map.get(int(sid), 0.0) for sid in sample_ids],
        dtype=np.float32,
    )
    ...
    if labeled_idx.size > 0:
        try:
            ...
            if f_lab_raw.shape[0] > 0:
                ...
                lab_score_ids, lab_entropy = strategy._score_avail(
                    model, labeled_idx, score_name="entropy"
                )
                lab_score_map = {
                    int(sid): float(s)
                    for sid, s in zip(lab_score_ids.tolist(), lab_entropy.tolist())
                }
                u_lab = np.array(
                    [lab_score_map.get(int(sid), 0.0) for sid in lab_ids],
                    dtype=np.float32,
                )
            ...
            log_strategy_action(
                f"UWE entropy: labeled seeding failed ({exc}), "
