"""HS-Cert-weighted coreset: hs_cert-scaled appearance embeddings + KMeans k-center."""
...
def run(strategy, k: int, model) -> np.ndarray:
    """Select k pairs by hs_cert-weighted coreset on appearance embeddings.

    Scales each pair's embedding by its hs_cert weight before running KMeans + k-center
    ...
    log_strategy_action(f"HS-Cert-weighted coreset: weighting {len(sample_ids)} embeddings by hs_cert.")
